...
(() => {
  ...
  function spotlightOn(target, click) {
    ...
    if (el) {
      ...
      // Keep the lit surface comfortably in view WITHIN the scroll region — below the top bar and
      // above the docked terminal (separate grid rows, so scrolling main can't hide behind them).
      // Land it about a THIRD of the way down — between the top and the centre — so the AI's focus
      // sits in reading space, not jammed under the top bar (owner 2026-07-17). A surface TALLER than
      // the viewport still top-aligns so it reads top-down. Scroll only when it isn't already fully
      // visible (a needless scroll cuts the lamp's glide short).
      ...
      if (r.top < region.top + 4 || r.bottom > region.bottom - 4 || tallerThanView) {
        if (tallerThanView) {
          el.scrollIntoView({ block: "start" });   // tall surface: top-align, readable top-down
        } else {
          const lead = region.height * 0.3;         // ~a third down = halfway between top and centre
          (main || document.scrollingElement || document.documentElement)
            .scrollBy({ top: (r.top - region.top) - lead });   // smoothness owned by CSS scroll-behavior
        }
      }
