...
# Deferred capability: mermaid→SVG + theme-tokened figures

> Folded in from the portfolio's former `MERMAID.md` (2026-07-06). Unbuilt. User-approved shape;
> canonical detailed plan also at `~/.claude/plans/eventual-bubbling-lemur.md` (memory
> `mill-mermaid-svg-plan`). Build in a separate worktree (Opus session).
...
- `mill/PLAN.md`: un-defer mermaid→SVG — tick it in "What MILL gives you" with the port/cache design one-liner.
